...
(function (root) {
  ...
  function aggregate(entries, filters) {
    const f = Object.assign({ includeAI: false, format: null, oppHero: null, period: 'all' }, filters || {});

    // Facettes (listes de valeurs) calculées sur TOUT, pour peupler les filtres.
    const formats = new Set(), oppHeroes = new Set();
    entries.forEach(e => {
    ...
    return {
      filters: f,
      facets: { formats: Array.from(formats).sort(), oppHeroes: Array.from(oppHeroes).sort() },
  ...
  function buildFilters() {
    ...
    ['#fltFormat', '#fltHero', '#fltPeriod', '#fltAI'].forEach(sel => {
