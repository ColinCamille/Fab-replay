...
(function (root) {
  ...
  // Agrégat « contexte » : alimente le carrousel des héros et la liste « tes
  // decks ». Il applique les filtres de TRANCHE (format · période · tag · IA)
  // mais PAS la sélection héros/adversaire (le carrousel EST le sélecteur de
  // héros ; l'adversaire est une dimension de matchup interne). Ainsi, filtrer
  // p.ex. « Classic Constructed » retire du carrousel les héros non joués dans
  // ce format. Les facettes (formats/tags) restent complètes : dans aggregate,
  // elles sont calculées sur toutes les entrées, pas sur les parties filtrées.
  function lifeAgg() { return aggregate(_entries, { includeAI: state.includeAI, format: state.format || null, period: state.period, tag: state.tag || null }); }
  ...
  function updateOppFacets() {
    ...
    // Adversaires proposés = ceux présents dans la tranche courante (héros
    // sélectionné + format + période + tag + IA), pour ne jamais offrir une
    // combinaison qui donnerait 0 partie.
    const f = { includeAI: state.includeAI, format: state.format || null, myHero: state.hero || null, period: state.period, tag: state.tag || null };
    const set = new Set();
    _entries.forEach(e => { if (passesFilters(e.record, f) && entryHasTag(e, f.tag)) { const o = oppHeroOf(e.record); if (o) set.add(o); } });
  ...
  function wire(host) {
    ...
    // Format / période / tag = filtres de tranche : ils redéfinissent aussi le
    // carrousel des héros et la liste des adversaires → re-rendu complet.
    host.querySelector('#hxFormat').addEventListener('change', e => { state.format = e.target.value; renderAll(); });
    // L'adversaire ne change ni le carrousel ni sa propre liste → re-rendu partiel.
    ...
    host.querySelector('#hxTag').addEventListener('change', e => { state.tag = e.target.value; renderAll(); });
    host.querySelector('#hxPeriod').querySelectorAll('button').forEach(b => b.addEventListener('click', () => { state.period = b.dataset.p; renderAll(); }));
